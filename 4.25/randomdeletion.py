...
def findDefination(astpath,solname):
    ...
    def_list = ["function","int","bool","bytes","_;","address","if","else","string","return","var","try","catch","event","emit","storage","memory"]
    if solname in data['output']['sources']:
...
def FindNoCoverage(htmlpath,solpath,dstpath,astpath,j):
    ...
    for line in lines:
        ...
        if count-1 >= 0 and count-1 < len(solline) and flag == 1 and line.find("cline-no") != -1:
            def_flag = 0
