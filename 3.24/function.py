...
def randomDeleteTest(srcpath,dstpath):
    ...
    eventastpath = os.path.join(eventsolpath,eventastfile[0])
    GenerateEvent.generateEvent(eventsolpath,eventastpath)
    #运行测试
    os.system("npx hardhat coverage --temp build")

    # 修改test文件夹下的所有文件，使其能够输出emit中的信息
    testpath = os.path.join(dstpath,"test")
    ...
    for testfile in testfiles:
        tf = open(os.path.join(dstpath,'test',testfile),"r")
        tflines = tf.readlines()
        tf.close()
        tflines[16] = teststr + tflines[16]
        tf = open(os.path.join(dstpath,"test",testfile),"w")
