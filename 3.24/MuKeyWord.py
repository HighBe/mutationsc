    ...
    while i < len(lines):
        ...
        if lines[i].find("bytes") != -1 and lines[i][lines[i].find("bytes")+5] != " "\
            ...
            if rr():
                ...
                while ii < len(lines_list) and lines_list[ii] in num:
                    lines_list.pop(ii)
        ...
        #    -> payable  payable与pure view 冲突
        if lines[i].find("function") != -1 and lines[i].find("//") == -1\
                and lines[i].find("pure") == -1 and lines[i].find("view") == -1\
                and lines[i].find(";") == -1 and lines[i].find("payable") == -1:
            if rr():
                lines_list = list(lines[i])
                if lines[i].find(")") != -1:
